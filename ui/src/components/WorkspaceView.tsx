...
export function WorkspaceView({
  ...
  const isControlled = integrateSidebar && typeof onSelectedPathChange === 'function'
  ...
  const markdownFiles = useMemo(() => {
    if (!files) return []
    return (files ?? []).filter((file: FileNode) => !file.is_dir && file.name.endsWith('.md'))
  }, [files])

  const effectiveSelectedPath = isControlled ? selectedPath : internalSelectedPath
  ...
  return (
    <div className="flex-1 h-full w-full bg-background text-foreground">
      <PanelGroup direction="horizontal" className="h-full">
        ...
        <Panel defaultSize={55} minSize={30} className="h-full">
          <Editor path={effectiveSelectedPath} onPathChange={updateSelectedPath} />
        </Panel>
        <PanelResizeHandle className="w-px bg-[var(--border-color)] transition-opacity hover:opacity-60" />
        <Panel
          ref={chatPanelRef}
